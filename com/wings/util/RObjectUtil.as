package com.wings.util
{
	...
	/**
	 * 提供对象的1些常用操作 clone和反射管理类
	 * http://code.google.com/p/ghostcat/
	 * @author flashyiyi
	 */
	
	public class RObjectUtil
	{
		
		
		
		/**
		 * 深度拷贝数据对象,注意不支持包内类复制,复杂自定义类对象(尤其带显示对象)慎用!!
		 * obj: 要拷贝的对象
		 * return ：返回obj的深度拷贝
		 * @param object
		 * @return
		 *
		 */
		public static function cloneDataObject(object:Object):Object
		{

			var tmpDisplayObj:DisplayObject=object as DisplayObject;
			if (tmpDisplayObj)
			{
				var filters:Array=tmpDisplayObj.filters;
				var colorTransform:ColorTransform=tmpDisplayObj.transform.colorTransform;
				var matrix:Matrix=tmpDisplayObj.transform.matrix;
			}
			...
			if (tmpDisplayObj)
			{
				tmpDisplayObj=copier.readObject() as DisplayObject;
				tmpDisplayObj.filters=filters;
				tmpDisplayObj.transform.colorTransform=colorTransform;
				tmpDisplayObj.transform.matrix=matrix;
				return tmpDisplayObj;
			}
			else
		...
		{	
			...
			{
				throw new Error("REF","反射失败！ReflectManager.eval() "+e.message);
		...
		public static function getDefinitionByName(name:String):Class
		{
			...
			try {
				...
				//trace("REF:反射类"+name+"失败！")
				//throw new Error("REF:反射类"+name+"失败！");
		 ...
		 * 
		 */		
		public static function isEqualeTypeof(instance:Object,objclass:Class):Boolean
		{
			if(instance && objclass)
			{
				return getQualifiedClassName(instance) == getQualifiedClassName(objclass);
			}
			else
				return false;
		}
