package com.wings.util.keyboard 
{
	...
	public class HotKey 
	{
		...
		private var _isIMEOpen:Boolean = false;
		...
		static public function get ctrlKey():Boolean
		{
			return _ctrlKey;
		}

		static public function get shiftKey():Boolean
		{
			return _shiftKey;
		}
		...
		static public function get altKey():Boolean
		{
			return _altKey;
		}
		...
		private function onKeyUp(key:KeyboardEvent):void {
			if (!_enabled || checkIsFromInputTextField(key.target)) return;
			...
			_shiftKey = key.shiftKey;
			_altKey = key.altKey;
			_ctrlKey = key.ctrlKey;
		}
		/**
		 * 有按键按下
		 * @param	key
		 ...
		 * 
		 */		
		private function execKeyDown(key:KeyboardEvent):void
		{
			_shiftKey = key.shiftKey;
			_altKey = key.altKey;
			_ctrlKey = key.ctrlKey;
		 ...
		 * 
		 */		
		private function checkIsFromInputTextField(obj:*):Boolean
		{			
			if (obj is TextField)
				return ((obj as TextField).type == TextFieldType.INPUT);
			else
				return false;	
		}
